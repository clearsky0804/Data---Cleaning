...
DROP TABLE IF EXISTS layoffstaging2;
...
DELETE FROM layoffstaging2 WHERE row_num > 1;  -- 5 entries which are duplicates
...
UPDATE layoffstaging2 AS t1   -- 3 entries where the industry was '' so updating them from the data we have
...
SELECT
	company, 
	SUM (CASE 
			WHEN total_laid_off <> 0 THEN total_laid_off
			ELSE 0
